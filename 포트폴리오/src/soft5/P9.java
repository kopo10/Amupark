...
public class P9 {
	...
	public static void main(String[] args) {
		...
		for (int k_10i = 0; k_10i < k_10itemname.length; k_10i++) { //i가 item크기만큼 돌 때
			...
			if (lengthdiff > 0 && k_10itemname[k_10i].getBytes().length > 18) { //lengthdiff가 0보다 크고 총 글자수를 9글자로(18바이트)로 하고 싶기 때문에 
				...
				for (int k_10k = 0; k_10k <= lengthdiff; k_10k++) { //lengthdiff차이만큼 반복해준다.
					...
					if (k_10itemname[k_10i].substring(0, 9 + k_10k).getBytes().length == 17) { //lengthdiff만큼 계속 substring 증가하다가 바이트가 17이 된다면
						if (k_10itemname[k_10i].substring(9 + k_10k, 9 + k_10k + 1).getBytes().length == 2) { // 두가지로 나눠서 그다음 글자 크기(한글)이 2이라면
							k_10itemname[k_10i] = k_10itemname[k_10i].substring(0, 9 + k_10k) + " "; //그대로 출력해주고 + " "을 해준다.
							// System.out.println("## " + k_10itemname[k_10i]); 
							break;

						} else {
							k_10itemname[k_10i] = k_10itemname[k_10i].substring(0, 9 + k_10k + 1); //바이트가 1(영어또는 특수문자)라면 이것까지 출력해준다.
							// System.out.println("@@ " + k_10itemname[k_10i]);
							break;

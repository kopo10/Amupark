...
public class Run {
	...
	public String Array(String DayOrNight, String jumin, String count, int typediscount, String Real_discount) {
		...
		if (Integer.parseInt(DayOrNight) == 1) {
			print += (int) (dayResidentNumber(jumin) * Discount(typediscount) * Integer.parseInt(count)) + "원  ";

		} else {
			print += (int) (nightResidentNumber(jumin) * Discount(typediscount) * Integer.parseInt(count)) + "원  ";
		}
		print += "*" + Real_discount + "\n";
		return print;
	}
